...
with st.sidebar:
    ...
    # Live mode backend selection
    st.markdown("**Live Mode**")
    available_backends = list_backends()
    backend_options = []
    backend_help = {}
    for name, usable, reason in available_backends:
        label = name if usable else f"{name} (unavailable)"
        backend_options.append(label)
        backend_help[label] = reason if not usable else ""

    selected_backend = st.radio(
        "Backend",
        options=backend_options,
        index=0,
        help="Select an LLM backend for live optimization. Unavailable backends are greyed out.",
    )
    # Determine if selected backend is usable
    selected_backend_name = selected_backend.split(" (")[0]
    backend_usable = any(
        name == selected_backend_name and usable
        for name, usable, _ in available_backends
    )

    optimize_clicked = st.button(
        "Optimize with AI",
        disabled=not task_input or not backend_usable or selected_backend_name == "simulation",
        help="Use the selected backend to generate an intent-optimized version of the task",
    )
...
# Handle live optimization
if optimize_clicked and backend_usable:
...
if "live_optimization" in st.session_state:
    ...
    st.caption(f"Generated by **{live['backend']}** backend")
    ...
    st.caption(
        f"Tokens used: {live['input_tokens']} input + {live['output_tokens']} output"
    )
